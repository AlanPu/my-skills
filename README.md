...
## fce-vocab-context
- 生成「FCE 词汇实战示范文」交互学习页——把 FCE / B2 First 高频词塞进一篇 90–120 词的短文，配中英双语、12 个词条精讲（音标 / 搭配 / 例句 / 易错点 / 升级替换）、写作迁移模板与高频易错点。覆盖 9 种体裁（人物传记 / 地点旅行 / 经历叙述 / 观点议论 / 建议信 / 评价评论 / 报告 / 活动节日 / 科技媒体），体裁与词表通过 `assets/pick.py` 随机轮换、互不重复。**支持导出适合打印的 PDF**（`assets/export_pdf.py`）：打印时自动切换浅色 A4 版式、收起右侧交互面板、把全部 12 个词条详解一次性铺开，并用实线/虚线区分两类词以保证黑白复印也分得清。当用户抱怨「背了 FCE 单词记不住、写作文不会用」，或要求「找一篇包含 FCE 单词的例文」「再来一篇 / 换个主题」「打印出来 / 导成 PDF」时调用。
